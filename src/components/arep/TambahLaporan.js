...
export default function TambahLaporan(props) {
  ...
  const onFinish = (values) => {
    ...
    formdata.append("to", "operator");
    formdata.append("title", "Laporan Baru");
    formdata.append("body", "Arep Mengupload Laporan Baru");
    axios
      .post(`/laporan/`, formdata, headers)
      .then((res) => {
        console.log(res);
        if (res.status === 200) {
          navigate("../StatusLaporan", {
            state: { id_arep: location.state.id_arep },
          });
        }
      })
      .catch((error) => {
